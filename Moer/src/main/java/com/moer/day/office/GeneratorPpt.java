...
import com.moer.day.office.util.Constants;
import com.moer.day.office.util.DateUtils;
import com.moer.day.office.util.PptConstants;
        ...
        if (isNull) {
            ...
            if (shapeData.getCtValAxMap() != null && shapeData.getCtValAxMap().size() > 0) {
                ...
                if (btCValAx != null) {
                    ...
                    // X轴 最大最小值 处理
                    if (bScaling != null) {
                        if (ctValAxMap.get(PptConstants.B_SCALING_MAX) != null) {
                            CTDouble max = bScaling.getMax();
                            if (max != null) {
                                max.setVal(ctValAxMap.get(PptConstants.B_SCALING_MAX));
                            } else {
                                bScaling.addNewMax().setVal(ctValAxMap.get(PptConstants.B_SCALING_MAX));
                            }
                        }
                        if (ctValAxMap.get(PptConstants.B_SCALING_MIN) != null) {
                            CTDouble mix = bScaling.getMin();
                            if (mix != null) {
                                mix.setVal(ctValAxMap.get(PptConstants.B_SCALING_MIN));
                            } else {
                                bScaling.addNewMin().setVal(ctValAxMap.get(PptConstants.B_SCALING_MIN));
                            }
                        }
                    }
                    // X轴 单位 处理
                    CTAxisUnit bMajorUnit = btCValAx.getMajorUnit();
                    if (ctValAxMap.get(PptConstants.B_MAJOR_UNIT) != null) {
                        if (bMajorUnit != null) {
                            bMajorUnit.setVal(ctValAxMap.get(PptConstants.B_MAJOR_UNIT));
                        } else {
                            btCValAx.addNewMajorUnit().setVal(ctValAxMap.get(PptConstants.B_MAJOR_UNIT));
                        }
                    }
                ...
                if (lCtValAxt != null) {
                    ...
                    if (ctValAxMap.get(PptConstants.L_SCALING_MAX) != null) {
                        CTDouble max = lScaling.getMax();
                        if (max != null) {
                            max.setVal(ctValAxMap.get(PptConstants.L_SCALING_MAX));
                        } else {
                            lScaling.addNewMax().setVal(ctValAxMap.get(PptConstants.L_SCALING_MAX));
                        }
                    }
                    if (ctValAxMap.get(PptConstants.L_SCALING_MIN) != null) {
                        CTDouble mix = lScaling.getMin();
                        if (mix != null) {
                            mix.setVal(ctValAxMap.get(PptConstants.L_SCALING_MIN));
                        } else {
                            lScaling.addNewMin().setVal(ctValAxMap.get(PptConstants.L_SCALING_MIN));
                        }
                    }

                    // Y轴 单位 处理
                    CTAxisUnit lMajorUnit = lCtValAxt.getMajorUnit();
                    if (ctValAxMap.get(PptConstants.L_MAJOR_UNIT) != null) {
                        if (lMajorUnit != null) {
                            lMajorUnit.setVal(ctValAxMap.get(PptConstants.L_MAJOR_UNIT));
                        } else {
                            lCtValAxt.addNewMajorUnit().setVal(ctValAxMap.get(PptConstants.L_MAJOR_UNIT));
                        }
                    }
                }
            }
    ...
    private static void fillSoildSrgColorForBubble(List<Map<String, Object>> specialValList, CTBubbleSer ctBubbleSer) {
        for (int j = 0; j < specialValList.size(); j++) {
            ...
            if (solidFillMap != null && solidFillMap.size() > 0) {
                Color solidFillSrgbClr = (Color) solidFillMap.get(PptConstants.SOLID_FILL_SRGB_CLR);
                String serIdx = (String) solidFillMap.get(PptConstants.PPT_SER_IDX);
                //填充的值不为空
